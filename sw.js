/* R69 — WALUTY KARTA 4 MASTER LIVE.
   KARTA 1, 2, 3: FINAL MASTER — BEZWZGLEDNIE BEZ ZMIAN.
   KARTA 4: zatwierdzony raster 852x1846 1:1; dynamiczne pola, wskazowka, PRZELICZ i LIVE.
   Bez HTML/SVG/canvasowych czarnych masek. Statyczne przykladowe znaki sa usuwane
   pikselowo z rastra przez lokalne uzupelnianie tekstury, bez widocznych prostokatow. */
importScripts('./sw-r66-stable.js?v=R66-final-surgical-stable');

const R69_K4_ASSETS=['./master-waluty-karta4-r69-master.webp'];
if(Array.isArray(ASSETS))R69_K4_ASSETS.forEach(a=>{if(!ASSETS.includes(a))ASSETS.push(a)});

const r69BasePatchIndexHtml=r48PatchIndexHtml;
r48PatchIndexHtml=function(text){
  let out=r69BasePatchIndexHtml(text);

  out=out.replaceAll('1.3.0-master-r66-waluty-karta3-final-surgical','1.3.0-master-r69-waluty-karta4-master-live');
  out=out.replaceAll('R66 WALUTY KARTA 3 FINAL SURGICAL','R69 WALUTY KARTA 4 MASTER LIVE');
  out=out.replace("const BUILD_DATE = '01.09.2026';","const BUILD_DATE = '02.09.2026';");
  out=out.replace("const BUILD_TIME = '16:28';","const BUILD_TIME = '05:12';");
  out=out.replace("navigator.serviceWorker.register('./sw.js?v=R66-waluty-karta3-final-surgical-1628'","navigator.serviceWorker.register('./sw.js?v=R69-waluty-karta4-master-live-0512'");

  out=out.replace(
    "img.src=card===3?'master-waluty-karta3-r64-clean.webp?v=R66-final-1628':`master-waluty-karta${card}.png?v=R43-LIVE-HARD-FIX`;",
    "img.src=card===3?'master-waluty-karta3-r64-clean.webp?v=R66-final-1628':card===4?'master-waluty-karta4-r69-master.webp?v=R69-master-live-0512':`master-waluty-karta${card}.png?v=R43-LIVE-HARD-FIX`;"
  );
  ...
  const k4=String.raw`  function renderCurrency4(){const {page,root}=wmBase(4);
    ...
    liveBtn.classList.add('r69-k4-live-hot');
    ...
    const mountK4Surface=()=>{
      if(!masterImg||!masterImg.naturalWidth||root.querySelector('.r69-k4-surface'))return;
      const W=852,H=1846;
      const surface=document.createElement('canvas');
      surface.className='r69-k4-surface';surface.width=W;surface.height=H;
      const ctx=surface.getContext('2d',{alpha:false});
      ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
      ctx.drawImage(masterImg,0,0,W,H);

      /* Pikselowe uzupelnianie tekstury: tylko jasne glify wewnatrz pol.
         Bez prostokatnych wypelnien, bez maskujacych DIV-ow, bez dodatkowych warstw graficznych. */
      const heal=(x,y,w,h,kind='all',dilate=2)=>{
        ...
        const pick=(px,py)=>{
          for(let rr=1;rr<=16;rr++){
            const cand=[[px-rr,py],[px+rr,py],[px,py-rr],[px,py+rr],[px-rr,py-rr],[px+rr,py+rr],[px-rr,py+rr],[px+rr,py-rr]];
            for(const q of cand){const qx=q[0],qy=q[1];if(qx>=0&&qx<w&&qy>=0&&qy<h&&!m[qy*w+qx])return (qy*w+qx)*4}
          } return -1;
        };
      ...
      /* Usuń WYŁĄCZNIE stara metaliczna igle z wnętrza zegara; hub i skala zostaja.
         Piksele sa uzupelniane najblizsza naturalna tekstura z tego samego wskaznika. */
      ...
      /* Jedna zywa wskazowka: -20 lewo, 0 gora, +20 prawo; poza zakresem zatrzymanie na skraju. */
      ...
      ctx.fillStyle='#101010';ctx.strokeStyle='#b7a879';ctx.lineWidth=3;ctx.beginPath();ctx.arc(cx,cy,23,0,Math.PI*2);ctx.fill();ctx.stroke();
      ctx.fillStyle='#050505';ctx.beginPath();ctx.arc(cx,cy,11,0,Math.PI*2);ctx.fill();ctx.restore();

      /* LIVE — wycinamy tylko dwa istniejace zielone glify strzalek z MASTER-a.
         Okrag i napis LIVE sa nieruchome; te same piksele obracaja sie zgodnie z zegarem. */
      ...
      lctx.putImageData(crop,sx,sy);sctx.putImageData(spr,0,0);
      ...
      if(currencyState.loading){
        ...
        const spin=now=>{if(!surface.isConnected||!currencyState.loading)return;ctx.drawImage(liveBase,0,0);const a=((now-started)/820)*Math.PI*2;ctx.save();ctx.translate(acx,acy);ctx.rotate(a);ctx.drawImage(sprite,-(acx-sx),-(acy-sy));ctx.restore();requestAnimationFrame(spin)};
    ...
    const buy=wmEdit(root,[399,390,214,77],currencyState.buyPrice,n=>currencyState.buyPrice=n,'r69-k4-edit r69-k4-input');
    wmSelect(root,[625,388,143,80],currencyState.buyCurrency,c=>currencyState.buyCurrency=c);
    wmDyn(root,[642,399,108,58],currencyState.buyCurrency,'gold r69-k4-code');
    wmSelect(root,[390,493,378,78],currencyState.buyCurrency,c=>currencyState.buyCurrency=c);
    wmDyn(root,[405,503,342,58],CURRENCY_FLAGS[currencyState.buyCurrency]+' '+currencyState.buyCurrency+' — '+CURRENCY_NAMES[currencyState.buyCurrency],'gold r69-k4-buycurrency');

    const sale=wmEdit(root,[399,597,214,78],currencyState.salePrice,n=>currencyState.salePrice=n,'r69-k4-edit r69-k4-input');
    wmSelect(root,[625,597,143,80],currencyState.saleCurrency,c=>currencyState.saleCurrency=c);
    wmDyn(root,[642,608,108,58],currencyState.saleCurrency,'gold r69-k4-code');

    const transport=wmEdit(root,[399,703,214,78],currencyState.transport,n=>currencyState.transport=n,'r69-k4-edit r69-k4-input');
    wmSelect(root,[625,703,143,80],currencyState.transportCurrency,c=>currencyState.transportCurrency=c);
    wmDyn(root,[642,714,108,58],currencyState.transportCurrency,'gold r69-k4-code');

    const spread=wmEdit(root,[399,809,214,78],currencyState.spreadPct,n=>currencyState.spreadPct=n,'r69-k4-edit r69-k4-input');
    wmHot(root,[625,809,143,80],()=>{spread.focus();spread.select();toast('SPREAD / PROWIZJA — wpisz wartość w %')},'Spread / prowizja');

    wmSelect(root,[625,941,143,79],outCur,c=>{currencyState.profitCurrency=c;curPersist();render()});
    wmDyn(root,[642,952,108,58],outCur,'gold r69-k4-code');
    wmSelect(root,[625,1044,143,79],outCur,c=>{currencyState.profitCurrency=c;curPersist();render()});
    wmDyn(root,[642,1055,108,58],outCur,'gold r69-k4-code');

    wmDyn(root,[443,947,180,65],curFmt(outCost,2),'gold r69-k4-cost');
    wmDyn(root,[443,1050,180,65],curFmt(outProfit,2),good?'green r69-k4-profit':'red r69-k4-profit');
    wmDyn(root,[92,1275,270,80],curFmt(v.margin,2)+'%',good?'green r69-k4-margin':'red r69-k4-margin');
    wmDyn(root,[91,1415,285,76],good?'OPŁACALNE':'NIEOPŁACALNE',good?'green r69-k4-status':'red r69-k4-status');

    const calcBtn=wmHot(root,[53,1534,746,128],()=>{
      [buy,sale,transport,spread].forEach(i=>i.blur());
      curPersist();
      calcBtn.classList.remove('r69-k4-calculated');void calcBtn.offsetWidth;calcBtn.classList.add('r69-k4-calculated');
      setTimeout(()=>{calcBtn.classList.remove('r69-k4-calculated');render();toast('✓ OPŁACALNOŚĆ PRZELICZONA')},520);
    },'Przelicz opłacalność');
    calcBtn.classList.add('r69-k4-calc');
  ...
  const r69Style=String.raw`<style id="r69-waluty-karta4-master-live">
...
.wm-page[data-card="4"] .r69-k4-surface{position:absolute!important;z-index:1!important;inset:0!important;width:100%!important;height:100%!important;display:block!important;pointer-events:none!important}
...
.wm-page[data-card="4"] .r69-k4-code{justify-content:flex-start!important;color:#ffd34c!important;font-size:clamp(17px,4.5vw,34px)!important;font-weight:900!important;line-height:1!important}
.wm-page[data-card="4"] .r69-k4-buycurrency{justify-content:flex-start!important;color:#ffd34c!important;font-size:clamp(12px,3.25vw,26px)!important;font-weight:850!important;line-height:1!important;white-space:nowrap!important}
.wm-page[data-card="4"] .r69-k4-cost,.wm-page[data-card="4"] .r69-k4-profit{justify-content:center!important;text-align:center!important;font-size:clamp(18px,4.8vw,38px)!important;font-weight:950!important;line-height:1!important}
.wm-page[data-card="4"] .r69-k4-margin{justify-content:flex-start!important;text-align:left!important;font-size:clamp(26px,7.2vw,58px)!important;font-weight:950!important;line-height:1!important}
.wm-page[data-card="4"] .r69-k4-status{justify-content:flex-start!important;text-align:left!important;font-size:clamp(17px,4.6vw,36px)!important;font-weight:950!important;line-height:1!important}
...
.wm-page[data-card="4"] .r69-k4-calc{border:0!important;border-radius:999px!important}
.wm-page[data-card="4"] .r69-k4-calc.wm-pressed,.wm-page[data-card="4"] .r69-k4-calc.r69-k4-calculated{background:rgba(118,255,0,.012)!important;box-shadow:0 0 17px 4px rgba(118,255,0,.62),inset 0 0 11px rgba(118,255,0,.12)!important;transform:scale(.998)!important}
</style>`;
  out=out.replace('</head>',r69Style+'\n</head>');
  return out;
};
