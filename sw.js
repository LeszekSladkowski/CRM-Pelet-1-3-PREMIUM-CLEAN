/* R130 v1.0E — AKCJE I STATUS — UPDATE ENGINE SYNC HOTFIX
   Baza wizualna: R130 v1.0D PIXEL MASTER FIT 1:1.
   Cel: zsynchronizować Service Worker, cache-busting, wersję aplikacji i loader warstwy LIVE.
   Raster MASTER 852x1846, silnik, dane, hotspoty oraz wcześniejsze MASTER-y pozostają nietknięte.
*/
importScripts('./sw-r127-language-base.js?v=R130-v1-0e-update-engine-sync-hotfix');
...
if(Array.isArray(ASSETS)){
  [
    ...
    './r130-akcje-status-live.js?v=R130-v1-0e-update-engine-sync-hotfix',
    './r130-akcje-status-layout-fix.js?v=R130-v1-0e-pixel-master-fit-1to1',
    ...
    './grafiki/rynki-eu/szczegoly-firmy/file_00000000d4dc820e99d86989f5522858.png?v=R130-v1-0e'
...
r48PatchIndexHtml=function(text){
  ...
  /* Jednoznaczna synchronizacja wersji — niezależnie czy klient startuje z R127, v1.0A/B/C/D. */
  [
  ...
  ].forEach(function(v){out=out.replaceAll(v,'1.3.0-master-r130v1-0e-update-engine-sync-hotfix');});
  [
  ...
  ].forEach(function(v){out=out.replaceAll(v,'R130 v1.0E AKCJE I STATUS — UPDATE ENGINE SYNC HOTFIX');});
  out=out.replace(/const BUILD_DATE = '[^']*';/,"const BUILD_DATE = '10.09.2026';");
  out=out.replace(/const BUILD_TIME = '[^']*';/,"const BUILD_TIME = '20:20';");
  out=out.replace(/sw\.js\?v=[^'\"]+/g,'sw.js?v=R130-v1-0e-update-engine-sync-hotfix-2020');
  out=out.replace(/r84-backup-prune\.js\?v=[^'\"]+/g,'r84-backup-prune.js?v=R130-v1-0e-2020');
  ...
  /* Skrypty LIVE — wymuszony cache-busting dla aktualnej warstwy R130 v1.0D. */
  ...
  out=out.replace(/<script src="\.\/r130-akcje-status-live\.js\?v=[^"]*"><\/script>/g,'<script src="./r130-akcje-status-live.js?v=R130-v1-0e-update-engine-sync-hotfix"></script>');
  if(!out.includes('r130-akcje-status-live.js?v=R130-v1-0e-update-engine-sync-hotfix'))out=out.replace('</body>','<script src="./r130-akcje-status-live.js?v=R130-v1-0e-update-engine-sync-hotfix"></script>\n</body>');

  out=out.replace(/<script src="\.\/r130-akcje-status-layout-fix\.js\?v=[^"]*"><\/script>/g,'<script src="./r130-akcje-status-layout-fix.js?v=R130-v1-0e-pixel-master-fit-1to1"></script>');
  if(!out.includes('r130-akcje-status-layout-fix.js?v=R130-v1-0e-pixel-master-fit-1to1'))out=out.replace('</body>','<script src="./r130-akcje-status-layout-fix.js?v=R130-v1-0e-pixel-master-fit-1to1"></script>\n</body>');
